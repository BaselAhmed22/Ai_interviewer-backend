...
def extract_text_from_file(file_path: str) -> str:
    ...
    if ext == ".pdf":
        ...
        # Skills/experience in a CV are often laid out as a table, not
        # paragraphs — doc.paragraphs alone silently drops that content.
        for table in doc.tables:
            for row in table.rows:
                cells = [cell.text.strip() for cell in row.cells if cell.text.strip()]
                if not cells:
                    continue
                # Merged cells repeat the same text across columns in
                # python-docx's row.cells — collapse consecutive repeats.
