...
def _balanced_difficulties(number_of_questions: int) -> list[str]:
    """A roughly even easy/medium/hard mix, shuffled — not left to
    Gemini, which tends to skew toward "medium" for everything."""
    difficulties = list(_DIFFICULTIES) * (number_of_questions // 3)
    remaining = number_of_questions - len(difficulties)
    if remaining > 0:
        difficulties.extend(random.sample(_DIFFICULTIES, remaining))
...
class QuestionnaireAgent:
    ...
    async def generate(
        ...
        for item in items:
            ...
            if text:
                valid.append((text, str(item.get("category", "") or "General").strip()))

        difficulties = _balanced_difficulties(len(valid))
        return [
            GeneratedQuestion(question=text, category=category, difficulty=difficulty)
            for (text, category), difficulty in zip(valid, difficulties)
        ]
